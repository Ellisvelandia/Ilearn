    ...
    <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 lg:gap-10 w-full z-10">
      ...
      >
        <div className="w-full">
          ...
          >
            <span className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl block bg-gradient-to-br from-foreground via-foreground/90 to-foreground/80 bg-clip-text text-transparent pb-2 drop-shadow-sm">
              AI-POWERED
            </span>
            <span className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl block bg-gradient-to-br from-foreground via-foreground/90 to-foreground/80 bg-clip-text text-transparent pb-2 drop-shadow-sm">
              STUDY TOOLS
        ...
        >
          Transform text into interactive study materials with AI-powered summaries, flashcards, and quizzes.
        </motion.p>
...
function HeroButtons({ prefersReducedMotion }: { prefersReducedMotion: boolean | null }) {
  return (
    <motion.div
      initial={prefersReducedMotion ? { opacity: 1 } : { opacity: 0, y: 20 }}
    ...
    >
      ...
      >
        <Link href="/auth/signup" className="flex items-center justify-center gap-2" aria-label="Get Started with AI Study Tools">
          Get Started
...
function HeroMetrics({ prefersReducedMotion }: { prefersReducedMotion: boolean | null }) {
  return (
    <motion.div
      initial={prefersReducedMotion ? { opacity: 1 } : { opacity: 0, y: 20 }}
      animate={prefersReducedMotion ? { opacity: 1 } : { opacity: 1, y: 0 }}
    ...
    >
      <div className="flex items-center gap-2 sm:gap-3">
        <span className="text-xl sm:text-2xl font-bold text-foreground/90 drop-shadow-sm">2x</span>
